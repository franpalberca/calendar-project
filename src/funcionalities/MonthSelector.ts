	...
	function writeMonth(month: number) {
		if (daysContainer) {
			...
			for (let i = 1; i <= currentMonthdaysContainer; i++) {
				...
				const toISODate: string = new Date(dateAttribute)
					.toISOString()
					.slice(0, 10);

				const numberDay = document.createElement("p");
				numberDay.setAttribute(
					"class",
					"d-flex px-2 mb-0 align-items-center justify-content-between"
				);
				numberDay.textContent = i.toString();
				dayElement.appendChild(numberDay);

				const dayEventContainer = document.createElement("div");
				dayEventContainer.setAttribute(
					"class",
					"container overflow-auto mh-75"
				);
				...
				addButton.addEventListener("click", (event: MouseEvent) => {
					if (event.target) {
						...
						const dayData = targetDay.getAttribute(
							"data-today"
						) as string;
			...
			const remainingNextDays =
				7 - ((startDayIndex + currentMonthdaysContainer) % 7);
